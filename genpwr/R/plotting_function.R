                  ...
                  select.Test.Model = NULL)
{
  ...
  for(j in 1:nrow(graphs))
  {
    ...
    plot_obj <- with(temp2, ggplot(aes(x=temp2[,x], y=N_total, group = Test.Model, colour = Test.Model))) +
            geom_line() +
                     ...
                     select.Test.Model = NULL)
{
  ...
  for(j in 1:nrow(graphs)){
    ...
    plot_obj <- with(temp2, ggplot(aes(x=temp2[,x], y=Power, group = Test.Model, colour = Test.Model))) +
      geom_line() +
      geom_point( size=4, shape=21, fill="white") + xlab(x)+ggtitle(subtitle)+
      facet_wrap( ~temp2[,panel.by])
    if(!is.null(y_limit)){
                    ...
                    select.Test.Model = NULL)
{
  ...
  for(j in 1:nrow(graphs)){
    ...
    plot_obj <- with(temp2, ggplot(aes(x=temp2[,x], y=OR, group = Test.Model, colour = Test.Model))) +
            geom_line() +
            geom_point( size=4, shape=21, fill="white") + xlab(x)+ggtitle(subtitle)+
            facet_wrap( ~temp2[,panel.by])
    if(!is.null(y_limit)){
