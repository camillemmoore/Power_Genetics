...
pw <- power.calc(N=c(5000, 20000), Case.Rate = 0.5, OR=1.5, Alpha=0.00000005, MAF=seq(0.01, 0.8, 0.005))
...
plot_obj_5000 <- ggplot(data = pw[pw$N_total==5000,], aes(x = pw[pw$N_total==5000, x], 
  ...
  geom_line() + 
  xlab(x) + geom_hline(aes(yintercept=0.8), linetype="dashed",color='black')+ facet_wrap(~temp2[, panel.by])
...
pdf('/Users/mooreca/Documents/genpwr_plots_OR_1.25_alpha_genome.pdf', height = 3, width=9)
...
cr <- Ncase/Ntotal
p_e <- (0.72*Ncase + 0.63*Ncontrol)/Ntotal
...
################################################
# OR_G = 5
pw_5 <- power_envir.calc(N = Ntotal, Case.Rate = cr, MAF = seq(0.05, 0.8, 0.05),
...
# OR_G = 1.5
pw_1.5 <- power_envir.calc(N = Ntotal, Case.Rate = cr, MAF = seq(0.05, 0.8, 0.05),
...
# OR_G = 1.25
pw_1.25 <- power_envir.calc(N = Ntotal, Case.Rate = cr, MAF = seq(0.05, 0.8, 0.05),
